var editor_btns = {'moreText':{'buttons':['bold','italic','underline','strikeThrough','fontSize','textColor','backgroundColor'],'buttonsVisible':7},'moreParagraph':{'buttons':['alignLeft','alignCenter','alignRight','alignJustify','formatOLSimple','formatULSimple','paragraphFormat','lineHeight','outdent','indent','quote'],'buttonsVisible':11},'moreRich':{'buttons':['insertImage','insertVideo','emoticons','insertLink','insertTable','fontAwesome','embedly','insertHR'],'buttonsVisible':8},'moreMisc':{'buttons':['undo','redo','html'],'align':'right','buttonsVisible':3}};var editor_opts = {heightMin:500,language:'zh_cn',toolbarButtons:editor_btns,toolbarContainer:'.article_editor_toolbar',events:{'contentChanged':getCharCount,'keydown':setEditFixed,'initialized':function(){setEditFixed();editor_init=true},'destroy':function(){editor_init=false}}};var editor_mobs = {heightMin:500,language:'zh_cn',theme:'dark',quickInsertEnabled:false,toolbarInline:true,toolbarVisibleWithoutSelection:true,toolbarButtons:[['bold','italic','underline','fontSize','textColor','backgroundColor']],events:{'contentChanged':getCharCount,'keydown':setEditFixed,'initialized':function(){setEditFixed();editor_init=true},'destroy':function(){editor_init=false}}};
var editor;
var editor_type = "??";
var editor_init = false;
function setEditFixed(){
	var el = $('.article_editor');
	var ws = $(window).scrollTop();
	var wh = $(window).height();
	var ft = ws-el.offset().top;
	var fb = (el.offset().top + el.outerHeight())-(wh+ws);
	$('.article_editor_toolbar').css('top',ft<=0?0:ft);
	$('.article_editor_savebar').css('bottom',fb<=0?0:fb);
}
function getCharCount(){
	var n = this.charCounter.count();
	$('.article_editor_charCount').text(n==''?0:n);
}
function initEditor(){
	var elem = $('.article_editor');
	var opts = editor_opts;
	if(elem.width()<768 && editor_type !== "xs"){
		if(editor_init) editor.destroy();
		editor = new FroalaEditor("#editor", editor_mobs);
		editor_type = "xs"
		console.log(editor_type);
	}
	if(elem.width()>=768 && elem.width()<860 && editor_type !== "sm"){
		if(editor_init) editor.destroy();
		opts.toolbarButtons.moreText.buttonsVisible = 4
		opts.toolbarButtons.moreParagraph.buttonsVisible = 3
		opts.toolbarButtons.moreRich.buttonsVisible = 2
		opts.toolbarButtons.moreMisc.buttonsVisible = 3
		editor = new FroalaEditor("#editor", opts);
		editor_type = "sm"
	}
	if(elem.width()>=860 && elem.width()<980 && editor_type !== "md"){
		if(editor_init) editor.destroy();
		opts.toolbarButtons.moreText.buttonsVisible = 4
		opts.toolbarButtons.moreParagraph.buttonsVisible = 6
		opts.toolbarButtons.moreRich.buttonsVisible = 1
		opts.toolbarButtons.moreMisc.buttonsVisible = 3
		editor = new FroalaEditor("#editor", opts);
		editor_type = "md"
	}
	if(elem.width()>=980 && elem.width()<1260 && editor_type !== "lg"){
		if(editor_init) editor.destroy();
		opts.toolbarButtons.moreText.buttonsVisible = 6
		opts.toolbarButtons.moreParagraph.buttonsVisible = 6
		opts.toolbarButtons.moreRich.buttonsVisible = 2
		opts.toolbarButtons.moreMisc.buttonsVisible = 3
		editor = new FroalaEditor("#editor", opts);
	...
	if(elem.width()>=1260 && editor_type !== "xlg"){
		if(editor_init) editor.destroy();
		opts.toolbarButtons.moreText.buttonsVisible = 7
		opts.toolbarButtons.moreParagraph.buttonsVisible = 11
		opts.toolbarButtons.moreRich.buttonsVisible = 8
		opts.toolbarButtons.moreMisc.buttonsVisible = 3
		editor = new FroalaEditor("#editor", opts);
		editor_type = "xlg"
	}
